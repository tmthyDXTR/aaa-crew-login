...
function generateRandomUsers(x) {
    const users = [];
    for (let i = 0; i < x; i++) {
...
const insertShiftSlots = (shiftId) => {
    ...
    connection.query(sql, [shiftId], (error, results, fields) => {
        ...
        while (currentSlotStart < end_time) {
            ...
            const insertSql = `INSERT INTO test_schichten_slots (test_schichten_id, start_time, end_time) VALUES (?, ?, ?)`;
            const values = [shiftId, currentSlotStart, currentSlotEnd];
...
exports.insertShiftSlots = insertShiftSlots;
